...
interface OverviewResponse {
  ...
  recentFiles: OverviewFile[];
  duplicates: OverviewDuplicate[];
}
  ...
  const runLiveRefresh = useCallback(
    async (trigger: "auto" | "manual") => {
      ...
      try {
        ...
        const bucketData = (await bucketsRes.json()) as {
          buckets?: BucketRef[];
        };
      ...
      if (failureCount === 0) {
        ...
        const prefix =
          trigger === "auto" ? "Auto refresh complete" : "Refresh complete";
        toast.success(
          `${prefix}: synced ${formatCount(syncedTotal)} file(s) across ${successCount} bucket(s)`,
        );
        return;
      }
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle>
              {formatCount(data.summary.indexedBucketCount)}
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle>{formatSize(data.summary.averageFileSize)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Active Tasks</CardDescription>
            <CardTitle>{formatCount(data.summary.activeTasks)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle>
              {formatCount(data.summary.distinctExtensionCount)}
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle>
              {formatSize(data.summary.multipartIncomplete.totalSize)}
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <p className="text-xs text-muted-foreground">
              {formatCount(data.summary.multipartIncomplete.uploads)} uploads,{" "}
              {formatCount(data.summary.multipartIncomplete.parts)} parts
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              scanned{" "}
              {formatCount(data.summary.multipartIncomplete.scannedBuckets)}{" "}
              bucket(s)
              {data.summary.multipartIncomplete.failedBuckets > 0 &&
                `, ${formatCount(data.summary.multipartIncomplete.failedBuckets)} failed`}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Last indexed object timestamp:{" "}
            {formatDate(data.summary.lastIndexedAt, "Never")}
          </CardDescription>
        </CardHeader>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Grouped using existing file type categories
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {data.types.map((entry) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {formatCount(entry.multipartIncompleteUploads)} incomplete
                      upload(s), {formatCount(entry.multipartIncompleteParts)}{" "}
                      part(s)
                    </p>
                  </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Extension-level distribution from indexed metadata
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
              ...
              <Table>
                ...
                <TableBody>
                  {data.largestFiles.map((file) => (
                    <TableRow key={`${file.bucket}/${file.key}`}>
                      <TableCell className="max-w-[200px] truncate" title={file.key}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Last 10 files by modification date</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="max-h-[360px] overflow-auto">
              <Table>
      ...
      {data.duplicates.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Files with identical size and extension (sorted by wasted space)
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {data.duplicates.map((d, i) => (
                    <TableRow key={i}>
                      <TableCell>{extensionLabel(d.extension)}</TableCell>
                      <TableCell className="text-right">{formatSize(d.size)}</TableCell>
                      <TableCell className="text-right">{formatCount(d.count)}</TableCell>
                      <TableCell className="text-right">
                        {formatSize(d.size * (d.count - 1))}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Click a bucket row to open explorer view
          </CardDescription>
        </CardHeader>
        <CardContent>
