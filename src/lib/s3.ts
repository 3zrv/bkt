...
export function normalizeS3Region(
  provider: string,
...
export function getSigningRegion(provider: string, region: string): string {
  const override =
    FIXED_SIGNING_REGION_PROVIDERS[provider.trim().toUpperCase()];
...
// Cache S3Client instances to reuse TCP connections across requests
const CLIENT_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes
const clientCache = new Map<
  string,
  { client: S3Client; endpoint: string; region: string; expiry: number }
>();
...
function getCachedClient(
...
): { client: S3Client; endpoint: string; region: string } {
  const now = Date.now();
  ...
  const { client, endpoint, region } = createS3ClientFromConfig(config);
  clientCache.set(credentialId, {
    client,
    endpoint,
    region,
    expiry: now + CLIENT_CACHE_TTL_MS,
  });
  return { client, endpoint, region };
}
...
export async function getS3Client(
  ...
  const { client, endpoint, region } = getCachedClient(credential.id, {
