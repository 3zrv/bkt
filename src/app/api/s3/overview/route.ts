...
export async function GET() {
  try {
    ...
    const [bucketGroups, extensionStats, fileAggregate, largestFiles, recentFiles, activeTasks, duplicateCandidates] = await Promise.all([
      ...
      prisma.fileMetadata.findMany({
        ...
        orderBy: { size: "desc" },
        take: 10,
        select: { key: true, bucket: true, size: true, lastModified: true },
      }),
      ...
      // Duplicate candidates: files with same size + extension appearing more than once
      prisma.$queryRaw<{ extension: string; size: bigint; count: bigint }[]>`
        SELECT "extension", "size", COUNT(*)::bigint AS "count"
        FROM "FileMetadata"
        WHERE "userId" = ${userId} AND "isFolder" = false AND "size" > 0
        GROUP BY "extension", "size"
        HAVING COUNT(*) > 1
        ORDER BY ("size" * COUNT(*))::bigint DESC
        LIMIT 20
      `,
    ])
    ...
    const indexedFileCount = fileAggregate._count._all
    const indexedTotalSize = toNumber(fileAggregate._sum.size)
    ...
    return NextResponse.json({
      summary: {
        ...
        indexedFileCount,
        indexedTotalSize,
        averageFileSize: indexedFileCount > 0 ? Math.round(indexedTotalSize / indexedFileCount) : 0,
